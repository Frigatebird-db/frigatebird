...
- `SELECT ... FROM <table>`
  - Single table, optional WHERE (supports `AND/OR`, comparisons, `BETWEEN`, `LIKE/ILIKE/RLIKE`, `IN`, `IS NULL`), arbitrary expression `ORDER BY` (with `ASC`/`DESC`, `NULLS FIRST`/`NULLS LAST`) plus `OFFSET`/`LIMIT`, aggregates in the projection, general `GROUP BY` with matching `HAVING`, and basic `DISTINCT`.
  - Projection list may mix plain columns with scalar expressions (arithmetic, CASE, built-in scalar functions) and the minimal window surface we’ve wired up so far: `ROW_NUMBER(...) OVER (...)` and `SUM(...) OVER (PARTITION BY ... ORDER BY ... ROWS UNBOUNDED PRECEDING)`.
